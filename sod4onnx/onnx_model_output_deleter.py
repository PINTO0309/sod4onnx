...
def outputs_delete(
    ...
    graph.outputs = [
        graph_output for graph_output in graph.outputs if graph_output.name not in output_op_names
    ]

    graph.cleanup().toposort()

    # Shape Estimation
    outputops_deleted_graph = None
    try:
        outputops_deleted_graph = onnx.shape_inference.infer_shapes(gs.export_onnx(graph))
    except:
        outputops_deleted_graph = gs.export_onnx(graph)
        if not non_verbose:
            print(
    ...
    if output_onnx_file_path:
        onnx.save(outputops_deleted_graph, output_onnx_file_path)

    if not non_verbose:
        print(f'{Color.GREEN}INFO:{Color.RESET} Finish!')

    # Return
    return outputops_deleted_graph
...
def main():
    ...
    parser.add_argument(
        ...
        required=True,
        help=\
            'Output name to be deleted from the models output OP. \n'+
            'e.g.\n'+
            '--output_op_names "output1" "output3"'
    )
    parser.add_argument(
    ...
    # Output OP add
    outputops_deleted_graph = outputs_delete(
